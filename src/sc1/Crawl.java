...
//			System.out.println("타입이름" + dd.select("a").attr("href").getClass().getName());
	...
	public List<News> getNewsList(List<String> reqList) throws Exception {
		...
		for (int i = 0; i < reqList.size(); i++) {
			if (!reqList.get(i).equals("#") && !reqList.get(i).contains("sports") && !reqList.get(i).contains("entertain")) {
				Document doc = Jsoup.connect(reqList.get(i)).get();
				
				Elements title = doc.select("#articleTitle");
				Elements date = doc.select("div.sponsor > span.t11");
				Elements contentBody = doc.select("#articleBodyContents");
				Elements media = doc.select("div.article_header div.press_logo a img");
...
//				news.setDate(date.first().text().split(". ")[0]);
				news.setContent(contentBody.text());
				news.setMedia(media.attr("title"));
